...
/**
 * verify-rot0.mjs
 * Computes and prints the final bounds of all meshes when rotationY = 0 is used.
 */
...
const scale = 0.049069;
const ox = 3.79 / 1000;
const oy = -60.72 / 1000;
const oz = 12.92 / 1000;

console.log('=== Final Vertex Bounds with rotationY = 0 (in meters) ===');
gltf.meshes.forEach((mesh, mi) => {
  ...
  mesh.primitives?.forEach((prim, pi) => {
    ...
    for (let i = 0; i < acc.count; i++) {
      ...
      // Scale then apply offset
      const fx = wx * scale + ox;
      const fy = wy * scale + oy;
      const fz = wz * scale + oz;

      if (fx < min[0]) min[0] = fx; if (fx > max[0]) max[0] = fx;
      if (fy < min[1]) min[1] = fy; if (fy > max[1]) max[1] = fy;
      if (fz < min[2]) min[2] = fz; if (fz > max[2]) max[2] = fz;
    }

    console.log(`Mesh ${mi} ("${mesh.name}"):`);
    console.log(`  Min:  [${min.map(v => (v * 1000).toFixed(1)).join(', ')}] mm`);
    console.log(`  Max:  [${max.map(v => (v * 1000).toFixed(1)).join(', ')}] mm`);
    console.log(`  Size: [${min.map((v, idx) => ((max[idx] - v) * 1000).toFixed(1)).join(', ')}] mm`);
  });
});
